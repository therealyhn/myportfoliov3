...
function NavItem({ children, index, mobile = false }) {
  ...
  return (
    <span className="group relative flex items-center gap-1.5 py-1 text-sm font-medium text-muted hover:text-ink transition-colors duration-200 cursor-pointer">
    ...
    <ul className={base}>
      ...
      <li onClick={onClick}>
        <a href="/JovanLjusicCV.pdf" download>
          <span className={
            mobile
              ? 'group flex items-center gap-3 text-base font-semibold text-accent cursor-pointer'
              : 'group relative flex items-center gap-1 py-1 text-sm font-semibold text-accent hover:text-accent/80 transition-colors duration-200 cursor-pointer'
          }>
            {mobile && <span className="w-5" />}
            Resume
